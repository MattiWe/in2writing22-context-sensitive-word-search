...
def _predict_with_bart(example_generator, model_name, device):  # "facebook/bart-large"
...
def _predict_with_bart_infilling(example_generator, model_name, device):
    ...
    # tokenizer = AutoTokenizer.from_pretrained(model_name, use_fast=False, add_prefix_space=True, pad_token="[PAD]", additional_special_tokens=["[MASK], [SEP], [1], [2], [3]"])
...
def _predict_with_clm_infilling(example_generator, model_name, device):
...
def _predict_with_clm(example_generator, model_name, device):
...
def _predict_with_mlm(example_generator, model_name, device):
    tokenizer = AutoTokenizer.from_pretrained(model_name)
...
def predict_from_pretrained(output_dir, model_name, model_type, input_dataset, device_id=0, batch_size=30,
    ...
    for results, batch in forward_function(input_generator, model_name, device):
